...
Привет, я умею читать статьи с https://nplus1.ru.
...
def send_article(message):
    ...
    if len(article) == 0:
        ...
        print(f'url: {sampled.url.values[0]}')
        bot.send_message(message.chat.id, sampled.date_published + ': ' + sampled.url.values[0])


@bot.message_handler(commands=['categories'])
def send_categories(message):
    markup = telebot.types.ReplyKeyboardMarkup(row_width=3)
    item_btn1 = telebot.types.KeyboardButton('категории Физика')
    item_btn2 = telebot.types.KeyboardButton('категории Математика')
    item_btn3 = telebot.types.KeyboardButton('категории Зоология')
